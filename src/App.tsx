...
function App() {
  ...
  /*
    Ricerca testuale libera: fino a ieri non c'era, e su un elenco lungo
    l'unico modo di arrivare a un task era scorrere. Vive fuori dai filtri
    salvati perche' e' una domanda del momento ("dov'e' quella cosa?"), non
    un modo di guardare il lavoro da conservare.
  */
  ...
  */
  ...
  const filteredAndSortedTasks = useMemo(() => {
    ...
    */
    ...
    /*
      La ricerca e' l'ultimo filtro ad applicarsi, sul gia' ristretto: cerca
      nel titolo e nella descrizione, senza distinzione di maiuscole. Il
      confronto e' su testo grezzo — niente fuzzy — perche' chi cerca "fattura"
      si aspetta i task che dicono "fattura", non quelli che le somigliano.
    */
    const queryRicerca = testoRicerca.trim().toLowerCase();
    if (queryRicerca) {
      filtered = filtered.filter(task =>
        task.title.toLowerCase().includes(queryRicerca) ||
        (task.description ?? '').toLowerCase().includes(queryRicerca)
      );
    }
    ...
    return sorted;
  }, [tasks, activeTab, filterStatus, filterPriority, filterDepartment, sortBy, employeesById, soloDaApprovare, currentEmployee, testoRicerca]);
  ...
  */
  ...
  }, [activeTab, filterStatus, filterPriority, filterDepartment, sortBy, soloDaApprovare, testoRicerca]);
  ...
  const tabEmployees = useMemo(() => {
    ...
    /*
      Una scheda per OGNI persona diventava un muro appena l'organico cresce, e
      chi non aveva niente assegnato occupava comunque una scheda per non
      mostrare nulla. Restano solo chi ha lavoro — piu' due eccezioni che non
      devono sparire: la persona della scheda aperta (sceglierla non deve
      cancellarla) e chi sta guardando (la propria scheda serve anche a zero
      task). In cima i piu' carichi, cosi' chi si cerca piu' spesso e'
      raggiungibile senza scorrere.
    */
    return Array.from(employeeMap.values())
      .filter(({ employee, taskCount }) =>
        taskCount > 0 ||
        employee.id === activeTab ||
        employee.id === currentEmployee?.id
      )
      .sort((a, b) =>
        b.taskCount - a.taskCount || a.employee.name.localeCompare(b.employee.name)
      );
  }, [employees, tasks, activeTab, currentEmployee?.id]);
      ...
      <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-8 max-w-7xl">
        ...
        {viewMode === 'dashboard' ? (
          ...
          */
          ...
          <div className="bg-card rounded-xl border p-4 sm:p-6 mb-6">
          {/*
            La ricerca sta sopra i filtri e a tutta larghezza: e' la prima cosa
            che si cerca quando l'elenco e' lungo, e non deve competere in
            larghezza con le tendine.
          */}
          <div className="relative mb-4">
            ...
            <Input
              ...
              placeholder={t('Search tasks by title or description')}

...
/*
  I codici di un fallimento di CONNESSIONE: la richiesta non e' mai arrivata al
  server, quindi nulla e' stato applicato e ritentare e' sicuro anche per una
  scrittura. `ECONNRESET` resta fuori apposta — puo' cadere a risposta gia'
  partita, e su un POST non sapremmo se ha toccato una riga.
*/
const CODICI_CONNESSIONE = new Set(['ECONNREFUSED', 'ETIMEDOUT', 'ENOTFOUND', 'EAI_AGAIN']);
...
async function chiamata(cfg, percorso, opzioni = {}) {
  ...
  /*
    La PRIMA fetch del processo, verso un indirizzo appena aperto, ogni tanto
    torna un ETIMEDOUT isolato che sparisce al tentativo dopo — e' un difetto
    noto dello stack di rete di Node quando prova piu' famiglie di indirizzi.
    Senza ritentare, la prima chiamata dopo l'avvio del connettore fallisce, e
    chi la riceve piu' in alto scambia quel guasto di rete per "sessione non
    piu' valida", dicendo all'utente di rifare l'accesso senza motivo. Si
    ritenta SOLO sull'errore di connessione, mai su una risposta del server: un
    rinnovo davvero scaduto e' un 4xx, arriva come risposta e continua a fallire
    subito.
  */
  let risposta;
  for (let tentativo = 0; ; tentativo++) {
    try {
      risposta = await fetch(`${cfg.url}${percorso}`, richiesta);
      break;
    } catch (errore) {
